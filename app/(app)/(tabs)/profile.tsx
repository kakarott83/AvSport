...
export default function ProfileScreen() {
  ...
  async function handleToggleHealth(enabled: boolean) {
      ...
      try {
        ...
        if (!availability.available) {
          if (availability.reason === 'sdk_unavailable') {
            ...
            Alert.alert('Health Connect veraltet', 'Bitte aktualisiere die Health Connect App im Google Play Store.');
          } else if (availability.reason === 'not_android') {
            Alert.alert('Nicht unterstützt', 'Health Connect ist nur auf Android verfügbar.');
          } else {
            Alert.alert('Fehler', 'Health Connect konnte nicht gestartet werden. Bitte versuche es erneut.');
          }
          return;
        }
        ...
        Alert.alert('Fehler', 'Health Connect konnte nicht aktiviert werden. Bitte versuche es erneut.');
        return;
      }
    }
    ...
    patch('sync_health', enabled);
    if (enabled) syncHealthToSupabase();
  }
        ...
        <View style={styles.card}>
          ...
          <View style={[styles.toggleRow, { marginBottom: form.cycle_tracking_enabled ? 20 : 0 }]}>
            <View style={styles.toggleLabelGroup}>
              ...
              <Text style={styles.toggleSub}>{form.cycle_tracking_enabled ? 'Aktiv' : 'Inaktiv'}</Text>
            </View>
            <Switch
          ...
          <View style={styles.card}>
            ...
            <View style={styles.toggleRow}>
              ...
              <Switch
                ...
                trackColor={{ false: '#2a2a2a', true: '#0a3a3a' }}
                thumbColor={!isNativeReady ? '#333' : form.sync_health ? '#00E5FF' : '#555'}
              />
            </View>
          </View>
        )}
...
const stepStyles = StyleSheet.create({
  ...
  btn:         { width: 44, height: 44, borderRadius: 12, backgroundColor: '#2a2a2a', borderWidth: 1, borderColor: '#333', alignItems: 'center', justifyContent: 'center' },
  btnDisabled: { opacity: 0.3 },
  btnText:     { color: '#fff', fontSize: 22, fontWeight: '300', lineHeight: 26 },
  btnTextDisabled: { color: '#555' },
  valueBox:    { flex: 1, flexDirection: 'row', alignItems: 'baseline', justifyContent: 'center', gap: 5, backgroundColor: '#1a1a1a', borderRadius: 12, borderWidth: 1, paddingVertical: 10 },
...
const styles = StyleSheet.create({
  ...
  chip:           { flex: 1, paddingVertical: 11, borderRadius: 10, backgroundColor: '#2a2a2a', alignItems: 'center', borderWidth: 1, borderColor: '#333' },
  ...
  toggleRow:        { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', backgroundColor: '#2a2a2a', borderRadius: 10, paddingHorizontal: 16, paddingVertical: 12, borderWidth: 1, borderColor: '#333' },
  ...
  saveBtn:        { backgroundColor: '#00E5FF', borderRadius: 14, padding: 16, alignItems: 'center', flexDirection: 'row', justifyContent: 'center', gap: 8 },
